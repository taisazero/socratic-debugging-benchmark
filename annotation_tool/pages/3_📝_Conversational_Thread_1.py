...
PAGE_NUMBER = 1
NEXT_PAGE_NAME = 'Conversational_Thread_2'
PREV_PAGE_NAME = 'Socratic_Dialogue'
PAGE_TITLE = 'Annotation Phase: Conversational Thread 1'
EXPORT_FILE_NAME = '_conversational_thread_1.txt'
NEXT_BUTTON = 'Socratic Thread 2 >'
PREV_BUTTON = '< Initial Dialogue'
...
def run_interface_ide():
    ...
    with c3:
        ...
        if next_button:
            if st.session_state.is_exported:
                st.session_state.load_page_from_exported_memory = True
                st.session_state.is_exported = False
                nav_page(NEXT_PAGE_NAME)
            else:
                chat_container.error('Please Save and Export the dialogue before proceeding to the next step.')
