...
def main(use_chat_prompt, generation_mode, num_responses, dataset_path, eval_method):
    ...
    chatgpt = ChatGPTModel(api_key=api_key, generation_args = generation_args, steering_prompt=steering_prompt)
    gpt4 = ChatGPTModel(model_name = 'gpt-4', api_key=api_key, generation_args = generation_args, steering_prompt=steering_prompt)
    ...
    if use_chat_prompt:
        chatgpt_path = f"results/chatgpt/chat_prompt_{generation_mode}_{n}_responses.xlsx"
        ...
        if generation_mode == 'multiple' or generation_mode == 'cot':
            ...
            gpt4_path = f"results/gpt4/instruction_prompt_{generation_mode}_{n}_responses.xlsx"
    ...
    for parsed_dialogue_object in tqdm(parsed_dialogues, desc="Processing dialogues"):
        ...
        if generation_mode == 'multiple':
            ...
            instruction = {
                    "first_introspection": "After looking at the buggy code, the bug description, and the big fix, what are all the possible reasons or misconceptions that led the user to make this mistake? Do NOT list Socratic questions.",
        ...
        for i, turn in enumerate(dialogue):
            ...
            if use_chat_prompt:
                if generation_mode == 'multiple':
                    raise ValueError("Multiple generation mode is not supported with chat prompt")
                else:
                    ...
                    messages.append(('User', context_text + "\n\n" + dialogue[0]['user_text']))

                    for j in range(1, i):
                        prev_turn = dialogue[j]
                        
                        if prev_turn['code_text'] != "":
                            messages.append(('User', prev_turn['user_text'] + "\n<code>\n" + prev_turn['code_text'] + "\n</code>\n"))
                        else:    
                    ...
                    chatgpt_response = chatgpt.generate_turn(messages, user_identifier='User', system_identifier='Assistant')
                    gpt4_response = gpt4.generate_turn(messages, user_identifier='User', system_identifier='Assistant')
            ...
            if generation_mode == 'multiple':
                ...
                if not chatgpt_response.startswith('1.') and not chatgpt_response.startswith('1. '):
                    chatgpt_response = '1.' + chatgpt_response if chatgpt_response.startswith(' ') else '1. ' + chatgpt_response
                if not gpt4_response.startswith('1.') and not gpt4_response.startswith('1. '):
                    gpt4_response = '1.' + gpt4_response if gpt4_response.startswith(' ') else '1. ' + gpt4_response
                # clean responses from any code blocks
                if '<code>' in chatgpt_response:
                    remove_code = extract_tag_content(chatgpt_response, 'code')
                    chatgpt_response = chatgpt_response.replace(f"<code>\n{remove_code}\n</code>", "")
                if '<code>' in gpt4_response:
                    remove_code = extract_tag_content(gpt4_response, 'code')
                    gpt4_response = gpt4_response.replace(f"<code>\n{remove_code}\n</code>", "")
    ...
    if eval_method != 'thoroughness':
        chat_gpt_scores = chatgpt_metrics.compute(chatgpt_responses, dataset_references, contexts=prompts)
        gpt4_scores = gpt4_metrics.compute(gpt4_responses, dataset_references, contexts=prompts)
    else:
        chat_gpt_scores = chatgpt_metrics.compute_thoroughness(chatgpt_responses, dataset_references, contexts=prompts)
        gpt4_scores = gpt4_metrics.compute_thoroughness(gpt4_responses, dataset_references, contexts=prompts)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--do_chat_prompt", action="store_true", help="Run chat prompt mode where the prompt is the chat history between the user and chatgpt. Defaults to an instructional prompt 'Respond to the user with a Socratic utterance that guides the user to discover and fix the bug described in `<bug_desc>`. Student code is denoted by `<code>` throughout the conversation. Utterances that have the same meaning but different words are considered duplicates. Assume that the student has run the test cases.'")
    # Add argument for generation mode. Either 'single', 'multiple', or 'sample'
    parser.add_argument("--generation_mode", type=str, default= "single", help="Generate a single response or multiple responses using instruction or sample k responses. Options are 'single', 'multiple', or 'sample'. Defaults to 'single'. 'multiple' is incompatible with '--do_chat_prompt'. If 'sample', the number of responses to sample is specified by the --num_responses argument.")
    parser.add_argument("--num_responses", type=int, default= 5, help="Number of responses to sample if --generation_mode is 'sample'. Defaults to 5.")
    parser.add_argument("--dataset_path", type=str, default= "reviewing_dialogues/eval_full_v2", help="Path to the dataset to evaluate. Defaults to 'reviewing_dialogues/eval_full_v2'.")
    ...
    if args.debug:
        ...
        main(args.do_chat_prompt, args.generation_mode, args.num_responses, args.dataset_path, args.evaluation_method)
        # debug()
