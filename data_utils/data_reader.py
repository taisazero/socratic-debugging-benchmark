...
class ExcelDataReader:
    def __init__(self, data_path):
        ...
        self.data.columns = ["worker_id", "task_id", 	"task_response_id", 'instruction', 'problem', 'faulty_code', 'bug_description', 'dialogue', 'resulting_code', 'comments']
    ...
    def process_dialogues_into_turns (self):
        ...
        for dialogue_ind, dialogue in enumerate(self.data['dialogue']):
            ...
            rolling_context = 'Programming problem: ' + problem + '\n' + \
                                'Code:\n' + faulty_code + '\n' + \
                                'Bug Description: ' + self.data['bug_description'][dialogue_ind].strip() + '\n' + \
            ...
            display_context = 'Programming problem: \n' + display_problem + '  \n' + \
                                'Faulty Code: \n' + faulty_code + '  \n' + \
                                'Bug Description: ' + self.data['bug_description'][dialogue_ind].strip() + '  \n' + \
                                self.data['instruction'][dialogue_ind].strip() + '  \n'
            ...
            for turn_ind , turn in enumerate(dialogue):
                ...
                if 'User: ' in turn:
                    ...
                    if dialogue[0].startswith('User: '):
                        if turn_ind + 1 < len(dialogue) and 'User: ' in dialogue[turn_ind + 1]:
                            assistant_utterance_end= dialogue[turn_ind + 1].index('User: ')
                            assistant_utterance = 'Assistant: ' + dialogue[turn_ind + 1][:assistant_utterance_end].strip()
                        elif turn_ind + 1 < len(dialogue) and 'User: ' not in dialogue[turn_ind + 1]:
                            assistant_utterance = 'Assistant: ' + dialogue[turn_ind + 1].strip()
                        else:
                            assistant_utterance = ''
                    elif turn[:user_utterance_index] != '':
                        assistant_utterance = 'Assistant: '  + turn[:user_utterance_index].strip()
                    else:
                        assistant_utterance = ''
                    user_utterance = turn[user_utterance_index:].strip()
                    user_utterance, thoughts, display_assistant_utterance = self.parse_thoughts(user_utterance, assistant_utterance, return_utterance=True)
    ...
    """
    def parse_thoughts(self, user_utterance, assistant_utterance, return_utterance=False):
        if user_utterance == '':
            return ''
        ...
        user_utterance = re.sub(r'<thought_block>.*?</thought_block>', '', user_utterance)
        assistant_utterance =  '<thought_block>' + thoughts.strip() + '</thought_block>'  + '  \n' + assistant_utterance 
        if return_utterance:
...
import regex as re
class XMLDataReader:
    def __init__(self, folder_path, tag_names =
                 ['dialogue', 'problem', 'bug_code', 'bug_desc', 
                  'bug_fixes', 'unit_tests', 'stu_desc']
                 ):
        ...
        for self.file_path in self.file_paths:
            ...
            with open(self.file_path, 'r') as f:
                ...
                dic = {
                        ...
                        'problem_id': self.file_path.split(os.sep)[-1].split('.')[0].split('_')[0],
                        'bug_id': self.file_path.split(os.sep)[-1].split('.')[0].split('_')[1]
                    }
    ...
    Returns:
        list: list of dictionaries with the keys:
            ...
            context: a dictionary with the following keys:
                ...
                stu_desc: student description
    """
    def get_parsed_dialogues (self, dialogue_tag_name='dialogue', exclude_tags = ['stu_desc']):
        returned_data = []
    ...
    def compute_dialogue_stats (self, dialogues_list):
        ...
        return {
            ...
            'total_utterances': sum(stats['assistant_main']) + sum(stats['user_main']) + sum(stats['assistant_alt']) + sum(stats['user_alt']),
    ...
    def _parse_dialogue(self, dialogue_text): 
        ...
        for l in lines:
            if l.strip().startswith('User:') or l.strip().startswith('Assistant:') or l.strip().startswith('<alt>') or l.strip().startswith('<code>') or l.strip().startswith('</code>'):
        ...
        for i, line in enumerate(lines):
            if line.startswith('User:'):
                ...
                if active_utternace == 'user':
                    user_alternatives.append({'alt_text': alt_text, 'alt_code': alt_code_text})
                elif active_utternace == 'assistant':
                    assistant_alternatives.append(alt_text)
                

            if user_text and (assistant_text or i == len(lines) - 1):
                if i < len(lines) - 1 and (lines [i + 1].startswith('Assistant:') or lines [i + 1].startswith('User:')) or i == len(lines) - 1:
                    
                    dialogue_data.append({
                        'user_text': user_text,
                        'code_text': code_text,
                        'cumulative_code_text': code_text if code_text != '' else dialogue_data[-1]['cumulative_code_text'] if len(dialogue_data) > 0 else '',
